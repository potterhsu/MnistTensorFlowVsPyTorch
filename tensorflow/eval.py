...
tf.app.flags.DEFINE_string('ckeckpoint_dir', './logs/train', 'Directory to read checkpoint files')
tf.app.flags.DEFINE_string('eval_logdir', './logs/eval', 'Directory to write evaluation logs')
FLAGS = tf.app.flags.FLAGS


def _eval(path_to_ckeckpoint_dir, path_to_eval_tfrecords_file, num_eval_examples, path_to_eval_log_dir):
    evaluator = Evaluator(path_to_eval_log_dir)

    checkpoint_paths = tf.train.get_checkpoint_state(path_to_ckeckpoint_dir).all_model_checkpoint_paths
...
def main(_):
    ...
    path_to_ckeckpoint_dir = FLAGS.ckeckpoint_dir
    ...
    _eval(path_to_ckeckpoint_dir, path_to_train_tfrecords_file, meta.num_train_examples, path_to_train_eval_log_dir)
    _eval(path_to_ckeckpoint_dir, path_to_val_tfrecords_file, meta.num_val_examples, path_to_val_eval_log_dir)
    _eval(path_to_ckeckpoint_dir, path_to_test_tfrecords_file, meta.num_test_examples, path_to_test_eval_log_dir)
